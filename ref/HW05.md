# HW 05 – Performance Testing

1. General Information

Exercise ID HW 05  AI
...
Dr. Lam Quang Vu / Dr. Tran Duy Hoang / MSc. Tran Thi

Lecturers & TAs

Bich Hanh / MSc. Truong Phuoc Loc / MSc. Ho Tuan Thanh

lqvu @ fit. hcmus. edu. vn / tdhoang @ fit. hcmus. edu. vn /

Contact ttbhanh @ fit. hcmus. edu. vn / tploc @ fit. hcmus. edu. vn /
...
G 9.1 → G 9.6, depending on the homework (see the CLO
...
Design and run Load, Stress, and Spike performance tests against the SUT ' s backend API using JMeter (or k 6).
...
Demonstrate Bloom - AI competencies at levels G 9.2  Apply , G 9.3  Analyse), G 9.4  Collaborate), and G 9.6  Disrupt).
...
The SUT exposes a REST backend API that the web frontend consumes;
...
Use three different report views. Across the three test plans, use three distinct listener / report types (e. g., View Results Tree, Summary Report, Aggregate Report); do not repeat a type.  JMeter terminology; k 6 users provide the equivalent distinct outputs.)
...
In your conclusion, propose a continuous performance - testing model that watches the SUT ' s commits, decides whether to run performance tests, and flags p 95 regressions. Include a flow chart and a discussion of the trade - offs (cost, false alarms).
...
The required Bloom - AI level for this homework is G 9.2  Apply , G 9.3  Analyse), G 9.4  Collaborate), and G 9.6  Disrupt).
...
Filename format: <StudentID></studentid>_HW05_AI_Performance_<SelfAssessedGrade></selfassessedgrade>.zip
